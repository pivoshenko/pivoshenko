...
This project is Python (3.13) managed with [`uv`](https://github.com/astral-sh/uv), and uses `just` to run its recipes.
...
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------- |
| `just install`                 | Installs every dependency group and extra into the project environment                                                                |
| `just format`                  | Rewrites the Python sources in place: modernizes syntax for Python 3.13+ with pyupgrade, then applies Ruff's autofixes and formatting |
| `just lint`                    | Lints the Python sources with Ruff and type-checks them with ty                                                                       |
| `just test`                    | Skips with a message if the `.no-tests` sentinel file is present, otherwise fails hard                                                |
| `just check`                   | Runs `lint` then `test`                                                                                                               |
| `just update`                  | Upgrades the lockfile to the newest allowed versions, then re-syncs the pins with uv-upsync                                           |
| `just update-readme-stats`     | Refreshes the stats section of `README.md` via `scripts/update_readme_stats.py` (needs `GH_TOKEN`)                                    |
| `just set-repository-policies` | Applies the standard settings across every repository on the account via `scripts/set_repository_policies.py` (needs `GH_TOKEN`)      |

1. Fork the repository and clone your fork locally
2. Create a branch following the [branch naming convention](#branches)
3. Make your changes, then run `just check` and confirm it passes before opening a pull request
